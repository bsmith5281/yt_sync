...
INPUTCSV = os.environ.get("INPUTCSV", "adventuremetadatapreview.csv")
OUTPUTCSV = os.environ.get("OUTPUTCSV", "adventure_metadata_generated_v8.csv")
...
ALL_CANONICAL_NAMES = set(HERO_ALIASES.values()) | set(BOSS_ALIASES.values())
ALL_NAME_LOOKUP = {}
...
def extract_names_with_scores(text, allowed_names=None):
    ...
    scores = Counter()
    candidates = allowed_names or ALL_NAME_LOOKUP.keys()
    ...
    for phrase in windows:
        for cand in candidate_list:
            ...
            if ratio >= 0.90:
                canonical = ALL_NAME_LOOKUP[cand]
...
def collect_evidence(row, transcript_text):
    ...
    for source_name, text in {
        ...
        "description": desc,
        "metadata": " ".join([
            row.get("currenthero", ""), row.get("currentboss", ""), row.get("heroname", ""),
            row.get("bossname", ""), row.get("suggestedtitle", ""), row.get("previousformattedtitle", ""),
            row.get("nextformattedtitle", ""), row.get("seriesname", ""), row.get("mode", "")
        ]),
...
def choose_pair(row, evidence):
    ...
    for source, names in evidence.items():
        for name in names:
            if name in set(HERO_ALIASES.values()):
                hero_votes[name] += 1
            if name in set(BOSS_ALIASES.values()):
                boss_votes[name] += 1
    ...
    title_pair_supported = current_hero and current_boss and current_hero in evidence["title"] and current_boss in evidence["title"]
    replacing_existing_boss = current_boss and chosen_boss and current_boss != chosen_boss
    metadata_conflict = replacing_existing_boss and current_boss in evidence["metadata"]
    ...
    if current_hero and current_boss and replacing_existing_boss and (metadata_conflict or filtered_boss_votes.get(chosen_boss, 0) < 2):
        return {
            "hero": current_hero,
            "boss": current_boss,
            "status": "preservedmatch",
            "reason": "preservedcurrentpairinsufficientevidencetooverwriteboss",
            "apply": "FALSE",
            "pairscore": str(max(pair_score, 0)),
            "preserved": "TRUE",
        }
    ...
    if chosen_hero and chosen_boss:
        ...
        if current_same:
            ...
            apply = "TRUE"
        elif pair_score >= 12 and filtered_boss_votes.get(chosen_boss, 0) >= 2:
            status = "needsupdate"
    ...
    if current_hero and current_boss:
        return {
            ...
            "boss": current_boss,
            "status": "uncertain",
            "reason": "keptcurrentpairinsufficientnewsignal",
            "apply": "FALSE",
            "pairscore": str(max(pair_score, 0)),
            "preserved": "TRUE",
        }
...
def build_final_title(hero, boss, row):
    ...
    existing = (row.get("suggestedtitle") or row.get("oldtitle") or row.get("title") or "").strip()
    return existing
...
def main():
    ...
    targets = [r for r in rows if looks_like_default(r) or FORMATTED_TITLE_RE.match((r.get("oldtitle") or r.get("title") or "").strip())]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for row in targets:
            ...
            row["sttconfidence"] = "high" if transcript_text and "transcript" in evidence else "none"
