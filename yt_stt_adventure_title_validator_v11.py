...
OUTPUTCSV = os.environ.get("OUTPUTCSV", "adventure_metadata_generated_v11.csv")
...
def choose_pair(row, evidence):
    ...
    metadata_conflict = bool(current_boss and chosen_boss and current_boss != chosen_boss and current_boss in evidence["metadata"])
    replacing_existing_pair = bool(current_hero and current_boss and chosen_hero and chosen_boss and (current_hero != chosen_hero or current_boss != chosen_boss))
    ...
    if current_hero and current_boss and chosen_hero == current_hero and chosen_boss == current_boss:
        return {
            "hero": chosen_hero,
            "boss": chosen_boss,
            ...
            "apply": "TRUE",
            "pairscore": str(max(pair_score, 12)),
            "preserved": "FALSE",
        }

    if replacing_existing_pair:
        if (
            chosen_hero not in HIGH_RISK_HEROES
            and hero_support >= 2
            and boss_support >= 3
            and pair_score >= 18
        ...
        return {
            ...
            "reason": "preservedcurrentpairinsufficientevidencetooverwritepair",
            "apply": "FALSE",
            "pairscore": str(max(pair_score, 0)),
            "preserved": "TRUE",
        }

    if chosen_hero and chosen_boss and not current_hero and not current_boss:
        if hero_support >= 2 and boss_support >= 2:
            return {
                "hero": chosen_hero,
                "boss": chosen_boss,
                "status": "verifiedmatch",
    ...
    return {
        "hero": chosen_hero or current_hero,
        "boss": chosen_boss or current_boss,
        "status": "uncertain",
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for row in targets:
            ...
            if decision["status"] == "preservedmatch":
                notes.append("conflicting replacement blocked")
            if decision["status"] == "uncertain":
                notes.append("manual review recommended")
            if decision["status"] == "needsupdate":
                notes.append("strong multisource replacement allowed")
            if decision["status"] == "verifiedmatch":
                notes.append("safe automatic formatting allowed")
