...
OUTPUT_CSV = "adventure_metadata_validated_v6.csv"
...
def build_title(row, hero, boss, mode):
    ...
    matchup = hero or boss
    return f"{matchup} – {series_name} | Hearthstone | Adventure Mode"
...
def classify_match_v6(row, hero, boss, metadata_supports_pair, metadata_conflicts_pair):
    family = (row.get("title_family") or "").strip().lower()
    ...
    # If metadata disagrees with directory resolution, do not auto-apply.
    if metadata_conflicts_pair:
        return "needs_review_risky"

    # MASS APPLICATION LOGIC:
    # 1) For structured adventure/book rows where metadata supports the pair, auto-apply regardless of STT.
    if family in ("formatted_book_of_heroes", "formatted_adventure_mode", "formatted_book_of_mercenaries", "other") \
       and hero and boss and metadata_supports_pair:
        return "auto_apply_mass"

    # 2) For default/contains_timestamp rows: if we resolved hero+boss with at least medium confidence, auto-apply.
    if family in ("default_timestamp", "contains_timestamp") and hero and boss and conf in ("high", "medium") and score >= 8:
        return "auto_apply_mass"

    # 3) Fallback: keep v5 slam-dunks as auto apply.
    if (
        status == "verified_match"
        and "current_title_matches_inferred_matchup" in row.get("verification_reason", "")
        and conf == "high"
        and score >= 14
    ):
        return "auto_apply_mass"
        ...
        for i, row in enumerate(rows, start=1):
            ...
            if video_id:
                try:
                    ...
                    # STT is now optional for mass application; we still record failure.
                    row["verification_status"] = "stt_failed"
                    row["verification_reason"] = f"stt_failed:{e}"
                    row["stt_raw"] = ""
                    row["stt_confidence"] = "none"
                    row["pair_score"] = "0"
                    # We do NOT early-continue; we still try to use metadata/directory.
                    transcript = ""

            mode, hero_inf, boss_inf, confidence, transcript_names, metadata_names, pair_score = resolve_matchup(row, transcript)

            # Prefer metadata hero/boss when present and valid in directory.
            ...
            # Resolve mode from metadata when possible.
            if meta_hero_key in HERO_TO_MODE:
                meta_mode = HERO_TO_MODE[meta_hero_key]
            else:
                meta_mode = mode

            # If metadata hero/boss exist and live in directory together, trust that pair.
            trusted_hero = ""
            trusted_boss = ""
            trusted_mode = meta_mode or mode

            if meta_mode in ADVENTURE_DIRECTORY:
                heroes = ADVENTURE_DIRECTORY[meta_mode].get("heroes", {})
                # Check if metadata pair is valid.
                for hero_candidate, boss_list in heroes.items():
                    if normalize_key(hero_candidate) == meta_hero_key and meta_boss_key in [normalize_key(b) for b in boss_list]:
                        trusted_hero = hero_candidate
                        # Canonical boss from list.
                        for b in boss_list:
            ...
            # Fall back to inference when we don't have a trusted metadata pair.
            ...
            # Build outputs from trusted values.
            ...
            metadata_supports_pair, metadata_conflicts_pair = metadata_pair_state(row, trusted_hero, trusted_boss)

            # Update verification_status / reason in a simpler, metadata-first way.
            if trusted_hero or trusted_boss:
                if current_title_matches(row, trusted_hero or "", trusted_boss or "", series_name):
                    ...
                    row["verification_reason"] = "current_title_matches_trusted_matchup"
                else:
                    row["verification_status"] = "needs_update"
                    reason_bits = [ "trusted_matchup_differs_from_current_title",
            ...
            # Decide auto-apply tier for v6.
            tier = classify_match_v6(row, trusted_hero, trusted_boss, metadata_supports_pair, metadata_conflicts_pair)

            if tier == "auto_apply_mass":
                row["needs_review"] = "FALSE"
                row["apply"] = "TRUE"
                row["review_notes"] = "auto_apply_mass_v6"
            elif tier == "needs_review_good":
                row["needs_review"] = "TRUE"
                row["apply"] = "FALSE"
                row["review_notes"] = "good_candidate_manual_ok"
            else:
                row["needs_review"] = "TRUE"
                row["apply"] = "FALSE"
            ...
            print(
                ...
                f"{trusted_hero} vs {trusted_boss} | score={pair_score} | conf={confidence} | tier={tier} | apply={row['apply']}"
            )
